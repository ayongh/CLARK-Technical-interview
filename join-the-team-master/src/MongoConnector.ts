...
export class MongoDriver 
{
  ...
  async addTasksToCategory(categoryId: string, taskList: string[]) {
    await this.categories.updateOne({ _id: new ObjectID(categoryId) }, { $push: { tasks: { $each: taskList.map(taskId => taskId) } } });
  }
  ...
  async removeTasksFromCategory(categoryId: string, taskList: string[]) {
    await this.categories.updateOne({ _id: new ObjectID(categoryId) }, { $pull: { tasks: { $in:taskList } } });
  }
